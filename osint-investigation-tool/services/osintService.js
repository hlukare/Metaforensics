...
class OSINTService {
  ...
  async generateReport(name, location = '', mainId = null, metadata = null) {
    try {
      // Execute all searches in parallel
      const [
        googleResults,
      ...
      const report = {
        ...
        metadata: metadata, // Add metadata to the report
        generated_at: new Date().toISOString()
      };
      ...
      // Format the report for clean output
      const formattedReport = this.reportFormatter.formatReport(reportWithIds);
      
      // Save the report if it's a new request
      if (!mainId) {
        await this.saveReport(main_id, sub_id, formattedReport);
      }
  ...
  async saveReport(mainId, subId, report) {
    try {
      ...
      let existingReport = await Report.findOne({ main_id: mainId });
      
      if (existingReport) {
        // Update existing report
        if (!existingReport.reports) {
          existingReport.reports = new Map();
        }
        existingReport.reports.set(subId, report);
        existingReport.last_updated = new Date();
        await existingReport.save();
      } else {
        // Create new report
        const newReport = new Report({
          main_id: mainId,
          reports: new Map([[subId, report]]),
      ...
      console.log(`Report saved to MongoDB: ${mainId}/${subId}`);
    } catch (error) {
  ...
  async getReport(mainId) {
    try {
      const report = await Report.findOne({ main_id: mainId });
      
      if (report) {
        // Convert Map to plain object for JSON serialization
        const reportsObj = {};
        if (report.reports) {
          report.reports.forEach((value, key) => {
            reportsObj[key] = value;
          });
        }
      ...
      console.error('Error reading report from MongoDB:', error);
      return { error: 'Failed to read report' };
    }
  }
}
