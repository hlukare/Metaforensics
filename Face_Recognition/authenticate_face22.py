...
def authenticate_from_json(input_data):
    try:
        if not isinstance(input_data, dict):
            return {"name": None, "location": "", "save_report": False}
        
        required_fields = ["image_path", "location", "save_report"]
        for field in required_fields:
            if field not in input_data:
                return {
                    "name": None,
                    "location": input_data.get("location", ""),
                    "save_report": input_data.get("save_report", False)
                }
        
        image_path = input_data["image_path"]
        location = input_data["location"]
        save_report = input_data["save_report"]
        
        if not os.path.exists(image_path):
            return {"name": None, "location": location, "save_report": save_report}
        ...
        return {"name": match_name, "location": location, "save_report": save_report}
        
    except Exception as e:
        return {
            "name": None,
            "location": input_data.get("location", "") if isinstance(input_data, dict) else "",
            "save_report": input_data.get("save_report", False) if isinstance(input_data, dict) else False
        }
